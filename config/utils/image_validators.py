...
def validate_image_resolution(image):
    ...
    if img_width > max_width or img_height > max_height:
        raise ValidationError('სურათის მაქსიმალური გაფართოება არის 4000x4000 პიქსელი')
    if img_width < min_width or img_height < min_height:
        raise ValidationError('სურათის მინიმალური გაფართოება არის 300x300 პიქსელი')
...
def validate_image_count(product_id):
    ...
    if count >= limit:
        raise ValidationError('1 პროდუქტზე დაშვებულია მხოლოდ 5 სურათის ატვირთვა')